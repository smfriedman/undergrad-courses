...
/**
 * Official CSE131 Lab 7 Student JUnit test.  
...
public class StudentAndCourseTest extends TestCase {
	...
	String s7 = "isGraduated() isn't working properly";
	...
	public void testIsGraduated() {
		...
		for (int i = 0; i < 119; ++i) {
			s.submitGrade(0,  1);
			assertEquals(s7, false, s.isGraduated());
		}

		for (int i = 0; i < 119; ++i) {
			s.submitGrade(0,  1);
			assertEquals(s7, true, s.isGraduated());
		}
	}

	@Test
	public void testHasHigherAverage() {
		Student s = new Student("D", "S", 1);
		s.submitGrade(2.0, 1);
		for (double i = 0; i <= 2; i+=0.1) {
			Student t = new Student("D", "S", 2);
			t.submitGrade(i, 1);
			assertEquals(s8, true, s.hasHigherAverage(t));
			assertEquals(s8, false, t.hasHigherAverage(s));
		}

		for (double i = 2.1; i <= 4; i+=0.1) {
			Student t = new Student("D", "S", 2);
			t.submitGrade(i, 1);
			assertEquals(s8, false, s.hasHigherAverage(t));
			assertEquals(s8, true, t.hasHigherAverage(s));
		}
	}
	...
	public void testComputeTuition() {
		...
		for (int i = 0; i < 26; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*2, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*3, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*4, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*5, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*6, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*7, s.computeTuition());
		}

		for (int i = 0; i < 18; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tution not working properly", 22850.0*8, s.computeTuition());
		}
	}
	...
	public void testCreateBaby() {
		for(int i = 0; i < 100; i++) {
			...
			assertTrue("create baby not creating ID properly", bb.getStudentID() >= 100000 && bb.getStudentID() <= 999999);
